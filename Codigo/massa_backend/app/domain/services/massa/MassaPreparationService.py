...
def normalizer_or_matrix(file, biological_activity):
    ...
    if len(biological_activity) == 1:
        bio_array = (file[biological_activity].to_numpy(copy=True)).reshape(-1,
                                                                            1)  # Transforming the list of biological activities into an array of shape -1:1.
    else:
        bio_array = file[biological_activity].to_numpy(
            copy=True)  # Transforming the list of biological activities into an array.
    ...
    descriptors_physicochemical = ['NumHAcceptors', 'NumHDonors', 'ExactMolWt', 'NumRotatableBonds', 'FractionCSP3',
                                   'TPSA', 'LogP_WildmanCrippen']
    PhCh_array = file[descriptors_physicochemical].to_numpy(
        copy=True)  # Insertion of physicochemical properties into an array.
    scaler = MinMaxScaler()  # Definition of the normalization parameter.
...
def organize_for_kmodes(
        file):  # Create a matrix with cluster identifications for each of the three domains, in order to prepare for Kmodes.
    kmodes_columns = file[['Cluster_Biological', 'Cluster_Physicochemical', 'Cluster_Structural']]
    dict_cluster_bio = {}
    dict_cluster_PhCh = {}
    dict_cluster_FP = {}
    for i, a, b, c in zip(kmodes_columns.index, kmodes_columns.Cluster_Biological,
                          kmodes_columns.Cluster_Physicochemical, kmodes_columns.Cluster_Structural):
