...
def split_train_test_sets(file, training_percent, test_percent):
    training, test = train_test_split(file, test_size=test_percent, train_size=training_percent, stratify=file[
        'Cluster_General'])  # Split the set of molecules into training set and test set, preserving the cluster diversity of the total set according to the stratification of the general clustering.
...
async def freq_clusters(file, extension_type, Cluster_Domain, barplot_xfont_size, result_id, session: AsyncSession):
    ...
    group1 = ax.bar(Total_ar - bar_width, percentages_total, bar_width, label='Total', color='black',
                    edgecolor='black')  # Bar plot for the total set.
    group2 = ax.bar(Training_ar, percentages_training, bar_width, label='Training set', color='dimgrey',
                    edgecolor='black')  # Bar plot for the training set.
    group3 = ax.bar(Test_ar + bar_width, percentages_test, bar_width, label='Test set', color='whitesmoke',
                    edgecolor='black')  # Bar plot for the test set.
